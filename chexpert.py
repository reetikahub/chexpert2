...
parser.add_argument('--evaluate_test_ensemble', action='store_true', help='Evaluate test model.')
...
parser.add_argument('--seed', type=int, default=1216, help='Random seed to use.')
...
def fetch_dataloader(args, mode):
    assert mode in ['train', 'valid', 'vis','test']
...
def fetch_dataloader_codalab(args, mode):
    assert mode in ['test_codalab']
    transforms = T.Compose([
        T.Resize(args.resize) if args.resize else T.Lambda(lambda x: x),
        T.CenterCrop(320 if not args.resize else args.resize),
        lambda x: torch.from_numpy(np.array(x, copy=True)).float().div(255).unsqueeze(0),   # tensor in [0,1]
...
def train_epoch(model, train_dataloader, valid_dataloader, loss_fn, optimizer, scheduler, writer, epoch, args):
    ...
    with tqdm(total=len(train_dataloader), desc='Step at start {}; Training epoch {}/{}'.format(args.step, epoch+1, args.n_epochs)) as pbar:
        for x, target, idxs in train_dataloader:
            ...
            #print(loss_fn(out, target.to(args.device)).sum(1).mean(0))
...
def evaluate_test_model(model, dataloader, loss_fn, args):
    outputs, targets, losses = evaluate(model, dataloader, loss_fn, args)
    return compute_metrics(outputs, targets, losses)
...
@torch.enable_grad()
def grad_cam(model, x, hooks,  model_name='vit', cls_idx=None):
    ...
    if model_name == 'vit':
        transform_weights = conv_features[0][:, 1:, :].reshape(conv_features[0].size(0), 14, 14, conv_features[0].size(2))
        transform_weights = transform_weights.transpose(2,3).transpose(1,2)
    else:
        transform_weights = conv_features[0]
    cam1 = F.relu(torch.sum(weights * transform_weights, dim=1, keepdim=True))

    # normalize each image in the minibatch to [0,1] and upscale to input image size
    cam = cam1.clone()  # avoid modifying tensor in-place
...
#    print(masks[0].shape)
    ...
    for attr, vis_idxs in zip(dataloader.dataset.vis_attrs, dataloader.dataset.vis_idxs):
        fig, axs = plt.subplots(5, 3, figsize=(4 * imgs.shape[1]/100, 7 * imgs.shape[2]/100), dpi=100, frameon=False)
        fig.suptitle(attr)
        ...
        filename = 'gradcam_{}_step_{}.png'.format(attr.replace(' ', '_'), args.step)
        plt.savefig(os.path.join(args.output_dir, 'vis', filename), dpi=100)
        plt.close()

def visualize_attn_masks(model, dataloader, args):
    attr_names = dataloader.dataset.attr_names
    ...
    for x, target, idx in dataloader:
        ...
        x = x.to(args.device)
        scores += [model(x).cpu()]
        attention_rollout = VITAttentionRollout1(model,attention_layer_name = layer_name,  head_fusion='mean', discard_ratio=0.7)
        mask, result = attention_rollout(x)
        masks += [mask]
    imgs, labels, scores, masks = torch.cat(imgs), torch.cat(labels), torch.cat(scores), torch.cat(masks)
    ...
    #for attr, vis_idxs in zip(dataloader.dataset.vis_attrs, dataloader.dataset.vis_idxs):
    #    fig, axs = plt.subplots(10, 3, figsize=(4 * imgs.shape[1]/100, 12 * imgs.shape[2]/100), dpi=100, frameon=False)
    #    fig.suptitle(attr)
    #    for i, idx in enumerate(vis_idxs):
    #        offset = idxs.index(idx)
    #        result = results.index(idx)
    #            for i, v in enumerate(result):
    #                # Attention from the output token to the input space.
    #                width = int(result[0].size(-1)**0.5)
    #                mask = v[0, 1:].reshape(width, width).detach().numpy()
    #                mask = cv2.resize(mask / mask.max(), (x[-1].size[1], x[-1].size[0]))[..., np.newaxis]
    #                mask = (mask * im).astype("uint8")
    #                fig, (ax1, ax2) = plt.subplots(ncols=2, figsize=(16, 16))
    #                ax1.set_title('Original')
    #                ax2.set_title('Attention Map_%d Layer' % (i+1))
    #                _ = ax1.imshow(im)
    #                _ = ax2.imshow(mask)
    #                filename = 'allatten_{}_step_{}.png'.format(i,args.step)
    #                plt.savefig(os.path.join(args.output_dir, 'vis', filename), dpi=100)
    #                plt.close()
    ...
    for attr, vis_idxs in zip(dataloader.dataset.vis_attrs, dataloader.dataset.vis_idxs):
        ...
        for i, idx in enumerate(vis_idxs):
            offset = idxs.index(idx)
            #print(imgs[offset].shape)
            #print(np.array(imgs[offset])[:, :, ::-1].shape)
            mask = masks[offset].detach().numpy()
            mask = cv2.resize(mask, (imgs[offset].shape[1], imgs[offset].shape[0]))
            visualize_one(model, imgs[offset], (mask), labels[offset], patient_ids[offset], probs[offset], attr_names, axs[i])

        filename = 'vis_{}_step_{}.png'.format(attr.replace(' ', '_'), args.step)
        plt.savefig(os.path.join(args.output_dir, 'vis', filename), dpi=100)
        plt.close()

def show_mask_on_image(img, mask, paper_cmap = False):
    heatmap = np.uint8(255*mask)
    heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
    heatmap = heatmap*0.5+img
...
def visualize_one(model, img, mask, label, patient_id, prob, attr_names, axs):
    ...
    axs[0].set_title(patient_id.rsplit("/")[2])
    data = np.stack([label, prob.round(3)]).T
    axs[0].table(cellText=data, rowLabels=names, colLabels=['Ground truth', 'Pred. prob'],
                 rowColours=plt.cm.Blues(0.5*label),
                 cellColours=plt.cm.Blues(0.5*data), cellLoc='center', loc='center')
    axs[0].axis('tight')
...
def vis_attn(x, patient_ids, idxs, attn_layers, args, batch_element=0, model_name = None):
    H, W = x.shape[2:]
    if model_name == 'vit':
        nh = attn_layers[0].heads
    elif model_name == 'vit_pretrained':
        nh = attn_layers[0].num_heads
    else:
        nh = attn_layers[0].nh
...
#        print(j,l)
        ...
        for ax, (ph, pw) in zip(axs[0], pix_to_vis(H,W)):
            image1 = x.clone().detach().mul_(0.0349).add_(0.5330).expand(-1,3,-1,-1)  # renormalize
            image = image1.clone()
        ...
        if model_name == 'vit' or model_name == 'vit_pretrained':
#            print(j)
            attn = l.attn.data[batch_element]
            attn = attn[:,1:,1:]
        else:
...
def rollout(attentions, discard_ratio, head_fusion):
    ...
    result = torch.eye(attentions[0].size(-1)).unsqueeze(0).repeat(attentions[0].shape[0], 1, 1)
    with torch.no_grad():
        for attention in attentions:
            ...
            flat[:, indices] = 0
            I = torch.eye(attention_heads_fused.size(-1)).unsqueeze(0).repeat(attentions[0].shape[0],1,1)
    ...
    mask = mask.reshape(mask.shape[0], width, width)
    mask = mask / torch.max(mask,(1,2), keepdims=True)
    #print(mask.shape)
    return mask    
...
def rollout1(attentions, discard_ratio, head_fusion):
    width = int(attentions[0].size(-1)**0.5)
    masks = torch.zeros((attentions[0].shape[0], width, width))
    for i in range(attentions[0].shape[0]):
        attentions_single = []
        for attention in attentions:
            attentions_single.append(attention[i])
        result = torch.zeros(len(attentions_single)+1, attentions_single[0].size(-1), attentions_single[0].size(-1))
        a_stack = torch.zeros(len(attentions_single), attentions_single[0].size(-1), attentions_single[0].size(-1))

        #print(result.shape)
        result[0] = torch.eye(attentions_single[0].size(-1))
        with torch.no_grad():
            for j, attention in enumerate(reversed(attentions_single)):
                attention = attention.unsqueeze(axis=0)
                ...
                a = (attention_heads_fused + 1.0*I)
                a = a / a.sum(dim=-1).unsqueeze(-1)
                print("a shape")
                print(a.shape)
                a_stack[j] = (attention_heads_fused)/attention_heads_fused.sum(dim=-1, keepdims=True)
                result[j+1] = torch.matmul(a, result[j])
        mask = result[-1]
    # In case of 224x224 image, this brings us from 196 to 14
        width = int(mask.size(-1)**0.5)
        mask = mask[0, 1:].reshape(width, width)
        mask = mask / torch.max(mask)
        fig, axs = plt.subplots(4, 3, figsize=(8,13), frameon=False)
        for j,v in enumerate(reversed(result[1:])):
            print("v shape {}".format(v.shape))
            mask_sep = v[0, 1:].reshape(width, width).detach().numpy()
            mask_sep = cv2.resize(mask_sep/mask_sep.max(), (224,224))
            _ = axs[int(j/3),j-3*int(j/3)].imshow(mask_sep)
            axs[int(j/3), j-3*int(j/3)].set_title('Layer_{}'.format(j), fontsize=10)
        filename = 'attn_vis_{}_step_{}.png'.format(i,j)
        plt.savefig(os.path.join(args.output_dir, 'vis', filename), dpi=100)
        plt.close() 
        masks[i] = mask
    return masks, result

class VITAttentionRollout1:
    def __init__(self, model, attention_layer_name='attend', head_fusion="mean",
        discard_ratio=0.9):
        self.model = model
        self.head_fusion = head_fusion
    ...
    def __call__(self, input_tensor):
        ...
        return rollout1(self.attentions, self.discard_ratio, self.head_fusion)
...
#        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [40000, 60000])
        ...
        if(args.pretrained):
            model = timm.create_model('vit_base_patch16_224_dino', pretrained=True, num_classes=n_classes, drop_rate=0.1, attn_drop_rate=0.2, drop_path_rate = 0.05, in_chans=1).to(args.device)
            grad_cam_hooks = {'forward': model.blocks[-1].norm1, 'backward': model.head}
        else:
            model = vit_model = ViT(image_size = 320, patch_size = 16, num_classes = n_classes, dim = 384, depth = 12, heads = 6, channels = 1, mlp_dim = 1536,dropout=0.1).to(args.device)
            #grad_cam_hooks = {'forward': model.blocks[-1].norm1, 'backward': model.fc}
#        grad_cam_hooks = None
        attn_hooks = []
        if args.pretrained == True:
            end_name = 'attn'
...
#                print(name)
                attn_hooks.append(module)
#        print(attn_hooks)

        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=1e-6)
#        optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, momentum=0.9, nesterov=True)
#        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [20000, 30000])
        scheduler = None
    elif args.model=='aadensenet121':
        model = DenseNet(32, (6, 12, 24, 16), 64, num_classes=n_classes,
                         attn_params={'k': 0.2, 'v': 0.1, 'nh': 8, 'relative': True, 'input_dims': (224,224)}).to(args.device)
    ...
    test_dataloader = fetch_dataloader(args, mode='test')
    ...
    class WeightedFocalLoss(nn.Module):
        ...
        def __init__(self, alpha=.25, gamma=2):
            ...
            self.alpha = torch.tensor([alpha, 1-alpha]).to(args.device)
            self.gamma = gamma

        # mean is the boolean to decide between returning the mean vs the whole vector of loss
        def forward(self, inputs, targets, mean=False):
            ...
            F_loss = F_loss.reshape(N, H)
            #print(f'size of F loss: {F_loss.size()}')
            if mean:
                return F_loss.mean()
            else:
                return F_loss
    ...
    loss_fn = WeightedFocalLoss().to(args.device)
    ...
    if args.evaluate_test_ensemble:
        eval_metrics = evaluate_ensemble(model, test_dataloader, loss_fn, args)
        print('Evaluate metrics -- \n\t restore: {} \n\t step: {}:'.format(args.restore, args.step))
        print('AUC:\n', pprint.pformat(eval_metrics['aucs']))
        print('Loss:\n', pprint.pformat(eval_metrics['loss']))
        save_json(eval_metrics, 'test_results_step_{}'.format(args.step), args)
...
#        print(model)
        ...
        if attn_hooks is not None:
            visualize_attn_masks(model, vis_dataloader, args)
            for x, _, idxs in vis_dataloader:
               ...
               #visualize stored attention weights for each image
                if args.model == 'vit':
                    if args.pretrained == True:
                        print("skip")
                        #for i in range(len(x)): vis_attn(x, patient_ids, idxs, attn_hooks, args, i, model_name='vit_pretrained')
                    else:
                        for i in range(len(x)): vis_attn(x, patient_ids, idxs, attn_hooks, args, i, model_name='vit')
                else:
