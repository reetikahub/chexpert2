...
from chexpert import fetch_dataloader, fetch_dataloader_codalab, load_json, densenet121, resnet152
...
# model params
parser.add_argument('--restore_path', default='./src/results/2022-05-30_06-41-34/', help='Path to a single model checkpoint to restore or path to folder of checkpoints to ensemble.')
parser.add_argument('--model', default='densenet121', choices=['densenet121', 'resnet152'], help='What model architecture to use.')
parser.add_argument('--cuda', default=0, help='Which cuda device to use.')
# dataloader params
parser.add_argument('--batch_size', type=int, default=16, help='Dataloader batch size.')
parser.add_argument('--resize', type=int, default=224, help='Size of minimum edge to which to resize images.')
...
parser.add_argument('--ensemble', action='store_false', help='Evaluate prediction output against dataseta single model.')
...
def predict(model, dataloader, args):
    ...
    for x, _, idxs in dataloader:
        ...
        patient_ids.append(extract_patient_ids(dataloader.dataset, idxs))
        columns = [*dataloader.dataset.attr_names]
        #print(patient_ids)
        #print(extract_patient_ids(dataloader.dataset, idxs))
    ...
    df = pd.DataFrame(data=probs, index=np.concatenate(patient_ids).ravel().tolist(), columns=[*dataloader.dataset.attr_names])
    df.index.name = 'Study'
...
if __name__ == '__main__':
    ...
    # load data
    dataloader = fetch_dataloader_codalab(args, mode='test_codalab')
    ...
    if args.debug:
        ...
        for _, target, idxs in dataloader:
            targets += [target]
            patient_ids.append(extract_patient_ids(dataloader.dataset, idxs))
        targets = pd.DataFrame(data=torch.cat(targets, 0).numpy(), index=np.concatenate(patient_ids).ravel().tolist(), columns=[*dataloader.dataset.attr_names])
