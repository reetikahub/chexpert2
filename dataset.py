...
#    print(dir_name)
    ...
    def __init__(self, root, mode='train', transform=None, data_filter=None, mini_data=None):
        ...
        assert mode in ['train', 'valid', 'test', 'vis', 'test_codalab']
        self.mode = mode
        ...
        if mode == 'test':
            ...
            data_file = os.path.join(self.root, self.dir_name, 'test.pt')
            self.data = torch.load(data_file)
            self.root = os.path.expanduser(root)  # base path; to be joined to filename in csv file in __getitem__
        ...
        elif mode == 'test_codalab':
            print(self.root)
            self.data = pd.read_csv(self.root, keep_default_na=True)
            self.root = '/home/ubuntu'  # base path; to be joined to filename in csv file in __getitem__
...
#            self._maybe_download_and_extract()
            ...
            if mode=='vis':
                ...
                for attr in self.attr_names:                                                               # 1 only condition category
                    idxs.append(self.data.loc[(self.data[attr]==1) & (self.data[self.attr_names].sum(1)==1), self.attr_names].head(n).index.tolist())
                idxs.append(self.data.loc[self.data[self.attr_names].sum(1)==0, self.attr_names].head(n).index.tolist())  # no findings category
                idxs.append(self.data.loc[self.data[self.attr_names].sum(1)==2, self.attr_names].head(n).index.tolist())  # 2 conditions category
                idxs.append(self.data.loc[self.data[self.attr_names].sum(1)>2, self.attr_names].head(n).index.tolist())   # >2 conditions category
    ...
    def _maybe_process(self, data_filter):
        ...
        if not (os.path.exists(train_file) and os.path.exists(valid_file) and os.path.exists(test_file)):
            ...
            test_df = self._load_and_preprocess_training_data(os.path.join(self.root, self.dir_name, 'test_ours.csv'), data_filter)
            valid_df = pd.read_csv(os.path.join(self.root, self.dir_name, 'valid.csv'), keep_default_na=True)
            train_df = self._load_and_preprocess_training_data(os.path.join(self.root, self.dir_name, 'train_ours.csv'), data_filter)
...
#    print(dataset.data['Path'].loc[idxs].str.rsplit('/', expand=True, n=1)[0])
#    print(dataset.data['Path'].loc[idxs].str.rsplit('/', expand=True, n=1)[0].values)
